function getDataForSearch() {
  ...
  return ws.getRange(2, 1, ws.getLastRow()-1, 3).getValues(); // row, column, numRows, numColumns
}
...
function getCustomerById(id){
  ...
  const posIndex = custIds.indexOf(id.toString().toLowerCase());
  const rowNumber = posIndex === -1 ? 0 : posIndex + 2;
  const customerInfo = ws.getRange(rowNumber, 1, 1, 9).getValues()[0];
  return { custID: customerInfo[0], firstName: customerInfo[1], lastName: customerInfo[2], email: customerInfo[3], tel: customerInfo[4], room: customerInfo[5], channel: customerInfo[6], start: customerInfo[7], end: customerInfo[8] }
